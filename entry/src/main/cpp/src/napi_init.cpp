...
struct FileChangeData : public NapiObject {
    FileChangeData(std::string_view fileId, std::string_view parentId, std::string_view path,
                   CloudDisk_OperationType opType, uint64_t sz, uint64_t mtimeVal)
...
struct SyncFileState {
    ...
    std::string ToString() const {
        return "SyncFileState{ name: " + name + ", syncState: " + magic_enum::enum_name(syncState).data() + " }";
    }
};
...
static void OnChangedCallback(const CloudDisk_SyncFolderPath syncFolderPath, const CloudDisk_ChangeData changeData[],
                              size_t bufferLength) {
...
static napi_value DoSetLocalFileState(napi_env env, napi_callback_info info) {
    ...
    if (canIUse("SystemCapability.FileManagement.CloudDiskManager")) {
        ...
        const auto ret = OH_CloudDisk_SetFileSyncStates(path, fileSyncStates.data(), fileSyncStates.size(),
                                                        &failedLists, &failedCount);
        LogI("returned: %{public}s", magic_enum::enum_name<CloudDisk_ErrorCode>(ret).data());
...
static napi_value Init(napi_env env, napi_value exports) {
    const napi_property_descriptor desc[] = {
        ...
        {"registerFileChangeMonitor", nullptr, DoRegisterFileChangeMonitor, nullptr, nullptr, nullptr, napi_default,
         nullptr},
